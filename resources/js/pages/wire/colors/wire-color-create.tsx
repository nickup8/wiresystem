...
export default function WireColorCreate() {
    ...
    const submit = () => {
        setIsSending(true);
        router.post(route('wireColors.store'), getValues(), {
            onSuccess: () => {
                form.reset();
                setIsSending(false);
            },
    ...
    return (
        <div className="border-sidebar-border/70 dark:border-sidebar-border relative overflow-hidden rounded-xl border p-4">
            <HeadingSmall title="Новый цвет провода" description="Заполните поля ниже.  Форма не чуствительна к регистру. Все поля обязательны" />
            <Form {...form}>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input placeholder="30" value={field.value} onChange={(e) => field.onChange(e.target.value)} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input placeholder="Черный" value={field.value} onChange={(e) => field.onChange(e.target.value)} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
