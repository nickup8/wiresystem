...
export default function WireSizeCreate() {
    ...
    const submit = () => {
        setIsSending(true);
        router.post(route('wireSizes.store'), getValues(), {
            onSuccess: () => {
                form.reset();
                setIsSending(false);
            },
    ...
    return (
        <div className="border-sidebar-border/70 dark:border-sidebar-border relative overflow-hidden rounded-xl border p-4">
            <HeadingSmall title="Новое сечение провода" description="Заполните поля ниже.  Форма не чуствительна к регистру. Все поля обязательны" />
            <Form {...form}>
                <form className="mt-4 flex flex-col gap-4" noValidate onSubmit={handleSubmit(submit)}>
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                <FormLabel>Код сечения</FormLabel>
                                <FormControl>
                                    <Input placeholder="039" value={field.value} onChange={(e) => field.onChange(e.target.value)} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input placeholder="0,75" value={field.value} onChange={(e) => field.onChange(e.target.value)} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
